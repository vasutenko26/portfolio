// Кейс «Telegram-ассистент»: обложка (1280×853) + 6 галерейных рамок (1600×900).
// Рендер 2x → суперсемплинг. Скрины берём из botshots/.
const { chromium } = require('playwright');
const sharp = require('sharp');
const fs = require('fs');
const path = require('path');

const SHOTS = path.join(__dirname, 'botshots');
...
const img = (f) => 'data:image/jpeg;base64,' + fs.readFileSync(path.join(SHOTS, f)).toString('base64');
...
// ---- обложка ----
const coverHtml = () => `<!doctype html><html><head><meta charset="utf-8"><style>${FONTS}
...
.t{font-family:'SG';font-weight:700;font-size:78px;line-height:.98;letter-spacing:-.02em;color:#F3F6FA}
...
.win{position:absolute;top:50%;left:48%;transform:translate(-48%,-50%) rotate(-2deg);width:540px;height:680px;
 border-radius:18px;overflow:hidden;border:1px solid #2C3542;background:#0B0E12;
 box-shadow:0 40px 90px -28px #000,0 0 0 1px rgba(61,214,140,.10)}
.win img{width:100%;height:100%;object-fit:cover;object-position:top center;display:block}
...
.who b{color:#E6EAF0;font-weight:600;display:block}
 ...
 <div class="in">
  <div class="l">
   <div class="kick"><b>//</b> ai assistant</div>
   <div class="t">Telegram<br>task assistant</div>
   <div class="sub">Gemini · Docker · self-hosted</div>
   <div class="tech">tasks · deadlines · reminders · 3× daily backup</div>
   <div class="tag"><span class="d"></span>AI breakdown + voice</div>
  </div>
  <div class="r"><div class="win"><img src="${img('shot-02.jpg')}"></div></div>
 </div>
...
.body{position:absolute;top:46px;left:0;right:0;bottom:0;display:flex;align-items:center;justify-content:center;background:#0B0E12}
.body img{max-width:100%;max-height:100%;object-fit:contain;display:block}
</style></head><body><div class="c"><div class="grid"></div>
 <div class="win"><div class="bar"><span class="dots"><i></i><i></i><i></i></span>
  <span class="cap">${caption}</span><span class="badge">● live bot</span></div>
  <div class="body"><img src="${img(file)}"></div></div>
</div></body></html>`;

const GAL = [
  ['shot-01.jpg', 'Main menu & commands'],
  ['shot-02.jpg', 'Natural-language input → AI task breakdown (Gemini)'],
  ['shot-03.jpg', 'Task card & shared dashboard'],
  ['shot-04.jpg', 'AI plan of the day'],
  ['shot-05.jpg', 'Progress: points, streak & chart'],
  ['shot-06.jpg', 'Hand-off to partner: accept, discuss, ping'],
];
...
(async () => {
  ...
  await render(coverHtml(), 1280, 853, path.join(OUTDIR, 'bot-cover.png'));
  let i = 1;
  for (const [f, cap] of GAL) { await render(galleryHtml(f, cap), 1600, 900, path.join(OUTDIR, `bot-g${i}.png`)); i++; }
  await b.close();
