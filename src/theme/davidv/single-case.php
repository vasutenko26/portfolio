<?php
/**
 * Единый шаблон кейса. Все поля — через ACF (davidv_field), с плейсхолдерами.
 */
...
<article class="case">
	...
	<header class="case-hero">
		<div class="wrap">
			<a class="back" href="<?php echo esc_url( home_url( '/#work' ) ); ?>"><?php echo davidv_icon( 'arrow', 'ico ico--sm flip' ); ?> все кейсы</a>
	...
	<div class="wrap case-body">
		...
		<div class="case-cols">
			<section class="case-block reveal" data-reveal>
				<p class="case-block__k">01 — Задача</p>
				<div class="prose"><?php echo $task ? wp_kses_post( wpautop( $task ) ) : '<p class="muted">Описание задачи скоро появится.</p>'; ?></div>
			</section>
			<section class="case-block reveal" data-reveal>
				<p class="case-block__k">02 — Что сделал</p>
				<div class="prose"><?php echo $approach ? wp_kses_post( wpautop( $approach ) ) : '<p class="muted">Описание решения скоро появится.</p>'; ?></div>
			</section>
			<section class="case-block reveal" data-reveal>
				<p class="case-block__k">03 — Результат</p>
				<div class="prose"><?php echo $result ? wp_kses_post( wpautop( $result ) ) : '<p class="muted">Результаты скоро появятся.</p>'; ?></div>
				<?php if ( $metric ) : ?><p class="case-metric"><?php echo esc_html( $metric ); ?></p><?php endif; ?>
			</section>
		</div>

		<!-- Галерея (сюда Playwright заливает скриншоты) -->
		<section class="case-gallery reveal" data-reveal id="gallery">
			<p class="kicker">// галерея</p>
			...
			<?php else : ?>
				<div class="gallery-grid gallery-grid--empty">
					<?php for ( $i = 0; $i < 3; $i++ ) : ?>
						<span class="shot shot--ph" aria-hidden="true"><?php echo davidv_icon( 'server' ); ?><em>скриншот демо</em></span>
					<?php endfor; ?>
				</div>
			<?php endif; ?>
		</section>
		...
		<!-- Низ: репозиторий + навигация -->
		<footer class="case-foot reveal" data-reveal>
			<?php if ( $repo ) : ?>
				<a class="btn btn--ghost" href="<?php echo esc_url( $repo ); ?>" target="_blank" rel="noopener">
					<?php echo davidv_icon( 'github', 'ico ico--sm' ); ?> Репозиторий
				</a>
			<?php endif; ?>
			<a class="btn btn--primary" href="<?php echo esc_url( home_url( '/#contact' ) ); ?>">Обсудить задачу <?php echo davidv_icon( 'arrow', 'ico ico--sm' ); ?></a>
		</footer>
